...
// ── CONFIG: edit surat di sini ──
const LETTER_PARAGRAPHS = [
  'Hei kamu yang cantik,',
  'Aku sering bingung harus mulai dari mana ketika ingin bercerita tentang perasaanku padamu. Tapi malam ini aku memberanikan diri — karena kamu layak mendengarnya, selengkap-lengkapnya.',
...
const SIGNATURE = '— dengan seluruh cintaku ♥'
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        el.style.animation = 'fadeUp 0.9s forwards'
        obs.unobserve(el)
      }
    }, { threshold: 0.1 })
    obs.observe(el)
      ...
      >
        ...
        <div style={{
          ...
          {LETTER_PARAGRAPHS.map((p, i) => (
            <p key={i} style={{ marginTop: i === 0 ? 0 : '1.5rem' }}>{p}</p>
          ))}
        </div>
